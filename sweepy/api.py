...
def list_sweepstakes(session: Session = Depends(get_session)):
    ...
    return [
        Sweepstakes(
            ...
            method=sweep.method,
            participant_names=[p.name for p in sweep.participants],
            ignore_longshots=sweep.ignore_longshots,
        )
        for sweep in all_sweepstakes
    ]
